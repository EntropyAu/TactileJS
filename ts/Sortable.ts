module Tactile {
  ...
  export class Sortable extends Container {
    ...
    private _updatePlaceholderPosition(
    ...
    private _updateChildTranslations(
      animate:boolean=true,
      complete?:() => void):void
    {
      let offset:number = 0;
      ...
      let index:number = 0;
      for (let el of this._siblingEls) {
        if (index === this.index) {
    ...
    public _resizeToExcludePlaceholder():void {
      if (this.el.style[this._directionProperties.dimension] === 'auto') {
        let dimension = this.el.clientHeight;
        let placeholderDimension = this.placeholder.outerSize[this._directionProperties.index];
        let attributes = { [this._directionProperties.dimension]: dimension - placeholderDimension }
        console.log("Sortable._resizeToExcludePlaceholder", this.el, attributes);
        Animation.set(this.el, attributes, this.drag.options.containerResizeAnimation, () => {
          this.el.style[this._directionProperties.dimension] = 'auto';
        });
      }
    }


    public _resizeToIncludePlaceholder():void {
      if (this.el.style[this._directionProperties.dimension] === 'auto') {
        let dimension = this.el.clientHeight;
        let placeholderDimension = this.placeholder.outerSize[this._directionProperties.index];
        let attributes = { [this._directionProperties.dimension]: dimension + placeholderDimension }
